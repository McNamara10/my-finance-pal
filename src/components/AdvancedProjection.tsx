import { AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, ReferenceLine } from "recharts";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Progress } from "@/components/ui/progress";
import { RecurringItem } from "@/hooks/useRecurringItems";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";

interface ProjectionData {
    month: string;
    balance: number;
    income: number;
    expenses: number;
    costOfLiving: number;
    net: number;
    events?: any[]; // Keep any[] or specific interface if exported
}

interface AdvancedProjectionProps {
    data5: ProjectionData[];
    data10: ProjectionData[];
    expenses: RecurringItem[];
    incomes: RecurringItem[];
}

const AdvancedProjection = ({ data5, data10, expenses, incomes }: AdvancedProjectionProps) => {
    const totalIncomes = incomes.filter(i => i.active).reduce((sum, i) => sum + i.amount, 0);
    const totalFixedExpenses = expenses.filter(e => e.active).reduce((sum, e) => sum + e.amount, 0);

    // Get simulation cost from first data point (it's constant for now)
    const simCost = data5.length > 0 ? data5[0].costOfLiving : 0;
    const totalExpenses = totalFixedExpenses + simCost;

    const renderContent = (data: ProjectionData[]) => (
        <div className="space-y-8 animate-fade-in">
            {/* 1. Main Chart Section */}
            <Card className="border-primary/20 shadow-lg bg-card/50 backdrop-blur-sm">
                <CardHeader className="flex flex-row items-center justify-between pb-2">
                    <div>
                        <CardTitle className="text-xl">Proiezione Saldo</CardTitle>
                        <p className="text-sm text-muted-foreground">Andamento stimato per i prossimi 12 mesi</p>
                    </div>
                    <div className="flex items-center gap-4 text-sm">
                        <div className="flex items-center gap-2">
                            <div className="w-3 h-3 rounded-full bg-primary"></div>
                            <span>Saldo Previsto</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <div className="w-3 h-3 rounded-full bg-success/50 border border-success border-dashed"></div>
                            <span>Entrata Mensile (€{totalIncomes})</span>
                        </div>
                    </div>
                </CardHeader>
                <CardContent>
                    <div className="h-[400px] w-full mt-4">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={data} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
                                <defs>
                                    <linearGradient id="colorBalance" x1="0" y1="0" x2="0" y2="1">
                                        <stop offset="5%" stopColor="hsl(var(--primary))" stopOpacity={0.3} />
                                        <stop offset="95%" stopColor="hsl(var(--primary))" stopOpacity={0} />
                                    </linearGradient>
                                </defs>
                                <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" vertical={false} />
                                <XAxis
                                    dataKey="month"
                                    axisLine={false}
                                    tickLine={false}
                                    tick={{ fill: 'hsl(var(--muted-foreground))', fontSize: 12 }}
                                    dy={10}
                                />
                                <YAxis
                                    axisLine={false}
                                    tickLine={false}
                                    tick={{ fill: 'hsl(var(--muted-foreground))', fontSize: 12 }}
                                    tickFormatter={(value) => `€${value}`}
                                />
                                <Tooltip
                                    contentStyle={{
                                        backgroundColor: 'hsl(var(--popover))',
                                        border: '1px solid hsl(var(--border))',
                                        borderRadius: '8px',
                                        boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)',
                                    }}
                                    itemStyle={{ color: 'hsl(var(--popover-foreground))' }}
                                    formatter={(value: number) => [`€${value.toLocaleString('it-IT')}`, 'Saldo']}
                                    labelStyle={{ color: 'hsl(var(--muted-foreground))', marginBottom: '0.5rem' }}
                                />
                                <ReferenceLine y={totalIncomes} stroke="hsl(var(--success))" strokeDasharray="3 3" label={{ position: 'right', value: `Entrata €${totalIncomes}`, fill: 'hsl(var(--success))', fontSize: 12 }} />

                                <Area
                                    type="monotone"
                                    dataKey="balance"
                                    stroke="hsl(var(--primary))"
                                    strokeWidth={3}
                                    fillOpacity={1}
                                    fill="url(#colorBalance)"
                                    activeDot={{ r: 6, strokeWidth: 0 }}
                                />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                </CardContent>
            </Card>

            {/* 3. Monthly Progress View ("Mese per mese") */}
            <Card className="border-border/50">
                <CardHeader>
                    <CardTitle className="text-lg">🗓️ Mese per mese</CardTitle>
                </CardHeader>
                <CardContent>
                    <div className="space-y-4">
                        {data.map((monthData, index) => {
                            const growth = index > 0 ? monthData.balance - data[index - 1].balance : monthData.net;
                            return (
                                <div key={index} className="bg-card hover:bg-secondary/20 transition-colors p-4 rounded-xl border border-dashed border-border/60">
                                    <div className="flex items-center justify-between mb-3">
                                        <div className="flex items-center gap-4">
                                            <div className="w-14 h-14 rounded-lg bg-secondary flex items-center justify-center font-bold text-sm text-center p-1 leading-tight">
                                                {monthData.month}
                                            </div>
                                            <Progress value={Math.min(100, (monthData.balance / (monthData.balance + 1000)) * 100)} className="w-32 md:w-64 h-2" />
                                        </div>
                                        <div className="text-right">
                                            <div className="text-xl font-bold">€{monthData.balance.toLocaleString('it-IT')}</div>
                                            <div className={`text-xs ${growth >= 0 ? 'text-success' : 'text-destructive'}`}>
                                                {growth >= 0 ? '+' : ''}€{growth.toLocaleString('it-IT', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} vs mese prima
                                            </div>
                                        </div>
                                    </div>

                                    {/* Detailed Breakdown Section */}
                                    <div className="mt-4 pt-4 border-t border-border/30 text-sm">
                                        <p className="text-xs text-muted-foreground mb-2 font-medium">
                                            {index === 0
                                                ? `Movimenti da Oggi fino al ${monthData.month}:`
                                                : `Movimenti dal ${data[index - 1].month} al ${monthData.month}:`}
                                        </p>
                                        <div className="space-y-1">
                                            {monthData.events && monthData.events.length > 0 ? (
                                                monthData.events.map((evt, idx) => (
                                                    <div key={idx} className="flex justify-between items-center text-xs opacity-80 hover:opacity-100 px-2 py-1 bg-secondary/10 rounded">
                                                        <div className="flex items-center gap-2">
                                                            <span className="text-muted-foreground opacity-70 w-8">{evt.date.getDate()}/{evt.date.getMonth() + 1}</span>
                                                            <span className={evt.type === 'income' ? 'text-success' : 'text-foreground'}>
                                                                {evt.type === 'income' ? '↗' : '↘'} {evt.name || (evt.type === 'income' ? 'Entrata' : 'Spesa')}
                                                            </span>
                                                        </div>
                                                        <span className={evt.type === 'income' ? 'text-success font-medium' : 'text-destructive'}>
                                                            {evt.type === 'income' ? '+' : '-'}€{evt.amount}
                                                        </span>
                                                    </div>
                                                ))
                                            ) : (
                                                <div className="text-xs text-muted-foreground italic pl-2">Nessun movimento previsto in questo periodo</div>
                                            )}
                                        </div>
                                    </div>
                                </div>
                            );
                        })}
                    </div>
                </CardContent>
            </Card>
        </div>
    );

    return (
        <div className="space-y-8 animate-fade-in">
            {/* 2. Breakdown Section ("Composizione Mensile") - SHARED */}
            <Card className="border-border/50">
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <div>
                            <CardTitle className="text-lg">💶 Composizione Mensile Standard</CardTitle>
                            <p className="text-xs text-muted-foreground">Voci attive ricorrenti (budget teorico)</p>
                        </div>
                        <div className="text-sm text-right">
                            <span className="text-muted-foreground mr-2">Uscite totali:</span>
                            <span className="font-bold text-destructive">€{totalExpenses}</span>
                            <span className="mx-3 text-border">|</span>
                            <span className="text-muted-foreground mr-2">Entrata:</span>
                            <span className="font-bold text-success">€{totalIncomes}</span>
                        </div>
                    </div>
                </CardHeader>
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        {/* Recurring Expenses Cards */}
                        {expenses.filter(e => e.active).map(expense => (
                            <div key={expense.id} className="bg-secondary/30 rounded-xl p-4 border border-border/50 relative overflow-hidden">
                                <div className="flex justify-between items-center mb-2">
                                    <span className="font-medium text-sm truncate">{expense.name}</span>
                                    <span className="font-bold text-destructive">€{expense.amount}</span>
                                </div>
                                <div className="text-[10px] text-muted-foreground mb-2 flex justify-between">
                                    <span>Giorno {expense.day}</span>
                                    {expense.start_date && (
                                        <span>Dal: {new Date(expense.start_date).toLocaleDateString()}</span>
                                    )}
                                </div>
                                {/* Progress bar logic: % of total expenses */}
                                <Progress value={(expense.amount / totalExpenses) * 100} className="h-1.5 bg-secondary" indicatorClassName="bg-destructive/80" />
                            </div>
                        ))}

                        {/* Simulation Cost Card */}
                        {simCost > 0 && (
                            <div className="bg-primary/5 rounded-xl p-4 border border-primary/20">
                                <div className="flex justify-between items-center mb-2">
                                    <span className="font-medium text-sm text-primary">Vita quotidiana</span>
                                    <span className="font-bold text-primary">€{simCost}</span>
                                </div>
                                <Progress value={(simCost / totalExpenses) * 100} className="h-1.5 bg-secondary" indicatorClassName="bg-primary" />
                            </div>
                        )}

                        {/* Savings/buffer Card */}
                        {totalIncomes > totalExpenses && (
                            <div className="bg-success/5 rounded-xl p-4 border border-success/20 lg:col-start-4">
                                <div className="flex justify-between items-center mb-2">
                                    <span className="font-medium text-sm text-success">Risparmio Previsto</span>
                                    <span className="font-bold text-success">+€{totalIncomes - totalExpenses}</span>
                                </div>
                                <div className="text-xs text-success/80 mt-2">
                                    Avance mensile disponibile
                                </div>
                            </div>
                        )}
                    </div>
                </CardContent>
            </Card>

            <Tabs defaultValue="day5" className="w-full">
                <TabsList className="grid w-full grid-cols-2 max-w-[400px] mx-auto mb-4">
                    <TabsTrigger value="day5">Vista al 5 del mese</TabsTrigger>
                    <TabsTrigger value="day10">Vista al 10 del mese</TabsTrigger>
                </TabsList>
                <TabsContent value="day5">
                    {renderContent(data5)}
                </TabsContent>
                <TabsContent value="day10">
                    {renderContent(data10)}
                </TabsContent>
            </Tabs>
        </div>
    );
};

export default AdvancedProjection;
